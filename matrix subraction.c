...
int main(void) {  // start of main 
  ...
  if (m != p || n != q) {  // checking the base condition to perform matrix multiplication
    ...
    for (i = 0; i < m; i++) {
      for (j = 0; j < n; j++) {
        printf("%d\t", a[i][j]);    // displaying the vlaues that are stored in thier respective index
      }
      printf("\n");
    }
    ...
    for (i = 0; i < p; i++) {
      for (j = 0; j < q; j++) {
        printf("%d\t", b[i][j]);   // displaying the vlaues that are stored in thier respective
      }
      printf("\n");
    }
